...
def lint(path: pathlib.Path, *, relative: bool = True) -> Issues:
    ...
    issues = dataclass_wizard.fromlist(Issue, data)
    if relative:
        for issue in issues:
            filename = pathlib.Path(issue.filename)
            issue.filename = str(filename.relative_to(path))
